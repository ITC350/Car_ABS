...
void communication::serial_printout(uint8_t dataSens1[], uint8_t dataSens2[], uint8_t dataSens3[], uint8_t dataSens4[], uint16_t dataTrig[])
{
  uint16_t datTime = 0;
    for (uint16_t i = 0; i < MAXDATAINPUT; i++) {
      datTime += 20;
      Serial.print(datTime);Serial.print(",");Serial.print((int)dataSens1[i]);Serial.print(",");Serial.print((int)dataSens2[i]);Serial.print(",");Serial.print((int)dataSens3[i]);Serial.print(",");Serial.print((int)dataSens4[i]);Serial.print(",");Serial.println(dataTrig[i]);
    }
}
